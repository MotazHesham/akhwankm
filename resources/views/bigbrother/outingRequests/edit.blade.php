...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("bigbrother.outing-requests.update", [$outingRequest->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <input type="hidden" name="big_brother_id" value="{{$outingRequest->big_brother->id ?? ''}}" id="">
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('outing_type') ? 'is-invalid' : '' }}" name="outing_type_id" id="outing_type_id" required onchange="checkvalue()">
                    ...
                    @endforeach
                        <option value="" @if($outingRequest->outing_type_id == null) selected @endif>{{trans('global.other')}}</option>
                </select>
            ...
            <div  @if($outingRequest->outing_type_id != null) style='display:none;' @endif id="other" >
                <div class="form-group">
                    <label class="required" for="other">أكتب نوع النشاط</label>
                    <input class="form-control {{ $errors->has('other') ? 'is-invalid' : '' }}" type="text" name="other"  value="{{ old('other', $outingRequest->other) }}"  >
                    @if($errors->has('other'))
                        <div class="invalid-feedback">
                            {{ $errors->first('other') }}
                        </div>
