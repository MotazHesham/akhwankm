...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.managersRatting.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.managers-rattings.update", [$managersRatting->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('member_of_the_board_of_directors') ? 'is-invalid' : '' }}" type="text" name="member_of_the_board_of_directors" id="member_of_the_board_of_directors" value="{{ old('member_of_the_board_of_directors', $managersRatting->member_of_the_board_of_directors) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('executive_director') ? 'is-invalid' : '' }}" type="text" name="executive_director" id="executive_director" value="{{ old('executive_director', $managersRatting->executive_director) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('brotherhood_specialist') ? 'is-invalid' : '' }}" name="brotherhood_specialist_id" id="brotherhood_specialist_id" required>
                    @foreach($brotherhood_specialists as $id => $entry)
                        <option value="{{ $id }}" {{ (old('brotherhood_specialist_id') ? old('brotherhood_specialist_id') : $managersRatting->brotherhood_specialist->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ManagersRatting::EVALUATION_OF_FRATERNITY_PROCEDURES_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('evaluation_of_fraternity_procedures') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="evaluation_of_fraternity_procedures_{{ $key }}" name="evaluation_of_fraternity_procedures" value="{{ $key }}" {{ old('evaluation_of_fraternity_procedures', $managersRatting->evaluation_of_fraternity_procedures) === (string) $key ? 'checked' : '' }} required>
                        <label class="form-check-label" for="evaluation_of_fraternity_procedures_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ManagersRatting::EVALUATION_OF_INTERVIEWS_WITH_THE_SPECIALIST_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('evaluation_of_interviews_with_the_specialist') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="evaluation_of_interviews_with_the_specialist_{{ $key }}" name="evaluation_of_interviews_with_the_specialist" value="{{ $key }}" {{ old('evaluation_of_interviews_with_the_specialist', $managersRatting->evaluation_of_interviews_with_the_specialist) === (string) $key ? 'checked' : '' }} required>
                        <label class="form-check-label" for="evaluation_of_interviews_with_the_specialist_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number_of_interviews') ? 'is-invalid' : '' }}" type="number" name="number_of_interviews" id="number_of_interviews" value="{{ old('number_of_interviews', $managersRatting->number_of_interviews) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ManagersRatting::THE_CONVENIENCE_OF_CHOOSING_A_BIGBROTHER_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('the_convenience_of_choosing_a_bigbrother') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="the_convenience_of_choosing_a_bigbrother_{{ $key }}" name="the_convenience_of_choosing_a_bigbrother" value="{{ $key }}" {{ old('the_convenience_of_choosing_a_bigbrother', $managersRatting->the_convenience_of_choosing_a_bigbrother) === (string) $key ? 'checked' : '' }} required>
                        <label class="form-check-label" for="the_convenience_of_choosing_a_bigbrother_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ManagersRatting::THE_QUALITY_OF_THE_TRAINING_PROGRAM_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('the_quality_of_the_training_program') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="the_quality_of_the_training_program_{{ $key }}" name="the_quality_of_the_training_program" value="{{ $key }}" {{ old('the_quality_of_the_training_program', $managersRatting->the_quality_of_the_training_program) === (string) $key ? 'checked' : '' }} required>
                        <label class="form-check-label" for="the_quality_of_the_training_program_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ManagersRatting::EVALUATE_THE_STUDY_OF_CHALLENGES_AND_FIND_SOLUTIONS_TO_HELP_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('evaluate_the_study_of_challenges_and_find_solutions_to_help') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="evaluate_the_study_of_challenges_and_find_solutions_to_help_{{ $key }}" name="evaluate_the_study_of_challenges_and_find_solutions_to_help" value="{{ $key }}" {{ old('evaluate_the_study_of_challenges_and_find_solutions_to_help', $managersRatting->evaluate_the_study_of_challenges_and_find_solutions_to_help) === (string) $key ? 'checked' : '' }} required>
                        <label class="form-check-label" for="evaluate_the_study_of_challenges_and_find_solutions_to_help_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ManagersRatting::DESIRE_TO_CONTINUE_THE_RELATIONSHIP_BETWEEN_BROTHERS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('desire_to_continue_the_relationship_between_brothers') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="desire_to_continue_the_relationship_between_brothers_{{ $key }}" name="desire_to_continue_the_relationship_between_brothers" value="{{ $key }}" {{ old('desire_to_continue_the_relationship_between_brothers', $managersRatting->desire_to_continue_the_relationship_between_brothers) === (string) $key ? 'checked' : '' }} required>
                        <label class="form-check-label" for="desire_to_continue_the_relationship_between_brothers_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ManagersRatting::INTERACTION_OF_THE_SMALL_BROTHER_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('interaction_of_the_small_brother') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="interaction_of_the_small_brother_{{ $key }}" name="interaction_of_the_small_brother" value="{{ $key }}" {{ old('interaction_of_the_small_brother', $managersRatting->interaction_of_the_small_brother) === (string) $key ? 'checked' : '' }} required>
                        <label class="form-check-label" for="interaction_of_the_small_brother_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ManagersRatting::HOW_WELL_THE_BROTHERHOOD_WORK_TEAM_DEALT_AND_INTERACTED_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('how_well_the_brotherhood_work_team_dealt_and_interacted') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="how_well_the_brotherhood_work_team_dealt_and_interacted_{{ $key }}" name="how_well_the_brotherhood_work_team_dealt_and_interacted" value="{{ $key }}" {{ old('how_well_the_brotherhood_work_team_dealt_and_interacted', $managersRatting->how_well_the_brotherhood_work_team_dealt_and_interacted) === (string) $key ? 'checked' : '' }}>
                        <label class="form-check-label" for="how_well_the_brotherhood_work_team_dealt_and_interacted_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('general_notes') ? 'is-invalid' : '' }}" name="general_notes" id="general_notes" required>{{ old('general_notes', $managersRatting->general_notes) }}</textarea>
