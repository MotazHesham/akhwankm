...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.datingSession.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.dating-sessions.update", [$datingSession->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="row">
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control select2 {{ $errors->has('specialist') ? 'is-invalid' : '' }}" name="specialist_id" id="specialist_id" required>
                        @foreach($specialists as $id => $entry)
                            <option value="{{ $id }}" {{ (old('specialist_id') ? old('specialist_id') : $datingSession->specialist->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control select2 {{ $errors->has('big_brother') ? 'is-invalid' : '' }}" name="big_brother_id" id="big_brother_id" required>
                        @foreach($big_brothers as $id => $entry)
                            <option value="{{ $id }}" {{ (old('big_brother_id') ? old('big_brother_id') : $datingSession->big_brother->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-3">
                    <label for="small_brother_id">{{ trans('cruds.datingSession.fields.small_brother') }}</label>
                    <select class="form-control select2 {{ $errors->has('small_brother') ? 'is-invalid' : '' }}" name="small_brother_id" id="small_brother_id"  >
                        @foreach($small_brothers as $id => $entry)
                            <option value="{{ $id }}" {{ (old('small_brother_id') ? old('small_brother_id') : $datingSession->small_brother->id ?? '') }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group col-md-3">
                    ...
                    <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $datingSession->date) }}" required>
                    @if($errors->has('date'))
                        <div class="invalid-feedback">
                            {{ $errors->first('date') }}
                        </div>
                ...
                <div class="form-group col-md-6">
                    ...
                    <textarea class="form-control {{ $errors->has('interview_notes') ? 'is-invalid' : '' }}" name="interview_notes" id="interview_notes" required>{{ old('interview_notes', $datingSession->interview_notes) }}</textarea>
                ...
                <div class="form-group col-md-6">
                    ...
                    <textarea class="form-control {{ $errors->has('recommendations') ? 'is-invalid' : '' }}" name="recommendations" id="recommendations" required>{{ old('recommendations', $datingSession->recommendations) }}</textarea>
