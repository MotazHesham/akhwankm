                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <label class="required"
                                for="big_brother_id">{{ trans('cruds.outingRequest.fields.big_brother') }}</label>
                            <select class="form-control select2 {{ $errors->has('big_brother') ? 'is-invalid' : '' }}"
                                ...
                                @foreach ($big_brothers as $id => $entry)
                                    <option value="{{ $id }}"
                                        {{ old('big_brother_id') == $id ? 'selected' : '' }}>
                                        {{ $entry }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control select2 {{ $errors->has('outing_type') ? 'is-invalid' : '' }}"
                                name="outing_type_id" id="outing_type_id" required>
                                @foreach ($outing_types as $id => $entry)
                                    <option value="{{ $id }}"
                                        {{ old('outing_type_id') == $id ? 'selected' : '' }}>{{ $entry }}
                                    </option>
                                @endforeach
                            </select>
