...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.big-brothers.update", [$bigBrother->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <input type="hidden" name="user_id" value="{{$bigBrother->user_id}}">
            <div class="row"> 
                <div class="col-md-4"> 
                ...
                <div class="col-md-4"> 
                    <div class="form-group">
                        <label class="required" for="specialist_id">{{ trans('cruds.bigBrother.fields.specialist') }}</label>
                        <select class="form-control select2 {{ $errors->has('specialist') ? 'is-invalid' : '' }}" name="specialist_id" id="specialist_id" required>
                            @foreach($specialists as $id => $entry)
                                <option value="{{ $id }}" {{ (old('specialist_id') ? old('specialist_id') : $bigBrother->specialist->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
                        ...
                        @endif
                        <span class="help-block">{{ trans('cruds.bigBrother.fields.specialist_helper') }}</span>
                    </div>
                </div>
            </div>
