...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.small-brothers.update", [$smallBrother->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <input type="hidden" name="user_id" value="{{$smallBrother->user_id}}">
            <div class="row">
                <div class="form-group col-md-6">
                    <label class="required" for="name">{{ trans('cruds.user.fields.name') }}</label>
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $smallBrother->user->name) }}" required>
                    @if($errors->has('name'))
                ...
                <div class="form-group col-md-6">
                    ...
                    <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $smallBrother->user->email) }}" required>
                    @if($errors->has('email'))
                        <div class="invalid-feedback">
                            {{ $errors->first('email') }}
                        </div>
                ...
                <div class="form-group col-md-6">
                    ...
                    <input class="form-control {{ $errors->has('identity_number') ? 'is-invalid' : '' }}" type="text" name="identity_number" id="identity_number" value="{{ old('identity_number', $smallBrother->user->identity_number) }}" required>
                ...
                <div class="form-group col-md-6">
                    ...
                    <input class="form-control date {{ $errors->has('identity_date') ? 'is-invalid' : '' }}" type="text" name="identity_date" id="identity_date" value="{{ old('identity_date', $smallBrother->user->identity_date) }}" required>
                ...
                <div class="form-group col-md-6">
                    ...
                    <input class="form-control date {{ $errors->has('dbo') ? 'is-invalid' : '' }}" type="text" name="dbo" id="dbo" value="{{ old('dbo', $smallBrother->user->dbo) }}" required>
                    @if($errors->has('dbo'))
                        <div class="invalid-feedback">
                            {{ $errors->first('dbo') }}
                        </div>
                ...
                <div class="form-group col-md-6">
                    ...
                    <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone" id="phone" value="{{ old('phone', $smallBrother->user->phone) }}">
                    @if($errors->has('phone'))
                        <div class="invalid-feedback">
                            {{ $errors->first('phone') }}
                        </div>
                ...
                <div class="form-group col-md-6">
                    ...
                    @foreach(App\Models\User::GENDER_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('gender') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="gender_{{ $key }}" name="gender" value="{{ $key }}" {{ old('gender', $smallBrother->user->gender) === (string) $key ? 'checked' : '' }} required>
                ...
                <div class="form-group col-md-6">
                    ...
                    @foreach(App\Models\User::DEGREE_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('degree') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="degree_{{ $key }}" name="degree" value="{{ $key }}" {{ old('degree', $smallBrother->user->degree) === (string) $key ? 'checked' : '' }} required>
...
<script>
    ...
    init: function () {
@if(isset($smallBrother->user) && $smallBrother->user->cv)
      var file = {!! json_encode($smallBrother->user->cv) !!}
...
<script>
    Dropzone.options.imageDropzone = {
        ...
        init: function () {
        @if(isset($smallBrother->user) && $smallBrother->user->image)
            var file = {!! json_encode($smallBrother->user->image) !!}
                this.options.addedfile.call(this, file)
