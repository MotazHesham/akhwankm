...
@section('styles') 
<link href="{{ asset('css/mailing-chat.css') }}" rel="stylesheet" />
@endsection
                                    ...
                                    <div class="chat-system">
                                        ...
                                        <div class="chat-box">
                                            ...
                                            <div class="overlay-video-call">
                                                <img src="{{asset('img/175x115.jpg')}}" class="video-caller" alt="video-chat">
                                                <div class="">
                                                    <div class="calling-user-info">
                                                        <div class="d-flex">
                                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-arrow-left-circle go-back-chat"><circle cx="12" cy="12" r="10"></circle><polyline points="12 8 8 12 12 16"></polyline><line x1="16" y1="12" x2="8" y2="12"></line></svg>
                                            ...
                                            <div class="chat-box-inner">
                                                ...
                                                <div class="chat-conversation-box">
                                                    <div id="chat-conversation-box-scroll" class="chat-conversation-box-scroll" style="scroll-behavior: smooth">

                                                        @foreach($conversations as $conversation)
                                                            <div class="chat" data-chat="person{{$conversation->id}}" data-conversationid="{{$conversation->id}}">
                                                            </div>
                                                        @endforeach
                                                ...
                                                <div class="chat-footer">
                                                    <div class="chat-input">
                                                        <form class="chat-form" action="javascript:void(0);" data-conversationid="0">
...
$("document").ready(function(){  
    ...
    function load_chat_data(l, s,conversationid){
            $.post('{{ route('smallbrother.chatting.show') }}', {_token:'{{ @csrf_token() }}',conversation_id:conversationid, limit:l,start:s}, function(data){
            $('.active-chat').prepend(data);
            if(data == ''){
                let endofchat = '<p class="alert alert-success text-center">End of Chat</p>';
                $('.active-chat').prepend(endofchat);
            }else{
                action = 'inactive';
            }
        });
    ...
    function loadmore(conversationid){
        console.log('here');
        if(action == 'inactive'){
            action = 'active';
            start = start + limit;
            setTimeout(function(){
                load_chat_data(limit, start,conversationid);
            }, 1000);
        }
    }
    // ---------------------------------------------------------
    ...
    function refresh_contacts(){
        $.post('{{ route('smallbrother.chatting.refresh_contacts') }}', {_token:'{{ @csrf_token() }}'}, function(data){
            $('.chat-system .people').html(null);
            $('.chat-system .people').html(data);
        }); 
    }
    ...
    $(document).on('click','.user-list-box .person', function(event) {
        if ($(this).hasClass('.active')) {
            ...
            $('.chat-form').attr('data-conversationid',conversationid);
            ...
            $('.chat[data-chat = '+findChat+']').html(null);
            $.post('{{ route('smallbrother.chatting.show') }}', {_token:'{{ @csrf_token() }}',conversation_id:conversationid}, function(data){
                $('.chat[data-chat = '+findChat+']').html(data); 
                refresh_contacts(); 
                const getScrollContainer = document.querySelector('.chat-conversation-box'); 
                getScrollContainer.scrollTop = 0; 
                $('.chat-conversation-box').animate({
                    scrollTop: $('.active-chat .bubble:last-child').position().top
                }, 'slow');
            });
        }
    ...
    $('.mail-write-box').on('keydown', function(event) {
        if(event.key === 'Enter') {
            ...
            var conversationid = $('.chat-form').attr('data-conversationid');
            $.post('{{ route('smallbrother.chatting.send') }}', {_token:'{{ @csrf_token() }}', message:chatMessageValue, conversation_id:conversationid}, function(data){
                console.log(data);
            });
            const getScrollContainer = document.querySelector('.chat-conversation-box');
...
<script>
    ...
    channel.bind('App\\Events\\ChattingMessages',function(obj){ 
        ...
        $messageHtml = '<div class="bubble you">' + obj['message'] + '</div>'; 
        var conversationid = obj['conversation_id'];  
        var findchat = $('.mail-write-box').parents('.chat-system').find('.active-chat'); 
        if($('.chat-form').attr('data-conversationid') == conversationid){
            console.log('first true');
            if(obj['user_id'] != '{{Auth::id()}}'){
                console.log('second true');
                var appendMessage = findchat.append($messageHtml);
            } 
        }
        const getScrollContainer = document.querySelector('.chat-conversation-box');
        getScrollContainer.scrollTop = getScrollContainer.scrollHeight; 
