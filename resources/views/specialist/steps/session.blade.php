    ...
    <form method="POST" action="{{ route('specialist.session_store') }}" enctype="multipart/form-data">
        ...
        <div class="form-group ">
            ...
            <select class="form-control select2 {{ $errors->has('small_brother') ? 'is-invalid' : '' }}" name="small_brother_id" id="small_brother_id"  >
                @foreach($small_brothers2 as $id => $entry)
        ...
        <div class="form-group">
            <label class="required"
                for="interview_notes">{{ trans('cruds.datingSession.fields.interview_notes') }}</label>
            <textarea class="form-control {{ $errors->has('interview_notes') ? 'is-invalid' : '' }}"
                name="interview_notes" id="interview_notes" required>{{ old('interview_notes') }}</textarea>
        ...
        <div class="form-group">
            <label class="required"
                for="recommendations">{{ trans('cruds.datingSession.fields.recommendations') }}</label>
            <textarea class="form-control {{ $errors->has('recommendations') ? 'is-invalid' : '' }}"
                name="recommendations" id="recommendations" required>{{ old('recommendations') }}</textarea>
